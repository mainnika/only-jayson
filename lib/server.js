...
var Server = function (methods, options) {
  ...
  this.options = jayson.Utils.merge(defaults, options || {});
...
Server.prototype.call = function (request, originalCallback) {
  ...
  maybeParse(request, this.options, function (err, request) {
    ...
    if (err) {
      ...
      return callback(jayson.Utils.response(error, undefined, undefined, self.options.version));
    }

    // is this a batch request?
    if (jayson.Utils.Request.isBatch(request)) {
    ...
    if (!jayson.Utils.Request.isValidRequest(request, self.options.version)) {
      error = self.error(Server.errors.INVALID_REQUEST);
      return callback(jayson.Utils.response(error, undefined, undefined, self.options.version));
    }
    ...
    var respond = function (error, result) {
      if (jayson.Utils.Request.isNotification(request)) return callback();
      var response = jayson.Utils.response(error, result, request.id, self.options.version);
    ...
    method.execute(self, request.params, function (error, result) {
      ...
      if (jayson.Utils.Response.isValidError(error, self.options.version)) {
...
Server.prototype._batch = function (requests, callback) {
  ...
  var wrapper = function (request, index) {
    ...
    return function () {
      if (jayson.Utils.Request.isValidRequest(request, self.options.version)) {
        ...
        responses[index] = jayson.Utils.response(error, undefined, undefined, self.options.version);
        maybeRespond();
      }
    };
  };
  ...
  var stack = requests.map(function (request, index) {
    ...
    if (jayson.Utils.Request.isBatch(request)) return null;
...
function maybeParse(request, options, callback) {
  if (typeof (request) === 'string') {
    jayson.Utils.JSON.parse(request, options, callback);
  } else {
